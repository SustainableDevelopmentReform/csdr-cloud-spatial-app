...
describe('report route', () => {
  ...
  it('rejects draft preview PDF generation for org creators', async () => {
    const creatorClient = createAppClient(
      await createSessionHeaders({
        email: 'report-creator@example.com',
        organizationRole: 'org_creator',
      }),
    )

    const createdJson = await expectJsonResponse<{ id: string }>(
      await creatorClient.api.v0.report.$post({
        json: {
          name: 'Creator preview report',
        },
      }),
      {
        status: 201,
    ...
    await expectJsonResponse(
      await creatorClient.api.v0.report[':id']['preview-pdf'].$post({
        param: { id: createdJson.data.id },
      }),
      {
        status: 403,
